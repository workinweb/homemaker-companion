...
import dictionary, {type DictionaryResults, type DicType, type TextType} from "~/dictionary/dictionaryLink";
import Link from "next/link";

export function SearchInput() {
    const {isOpen, onOpen, onClose} = useDisclosure();
    const [searchQuery, setSearchQuery] = React.useState<string>('')
    const [foundSearch, setFoundSearch] = React.useState<DictionaryResults[] | []>([])
    ...
    function extractWords(text: string, trigger: string) {
        ...
        extracted = extracted.map(word => word.replace(new RegExp(`(${trigger})`, 'g'), '<span class="text-white bg-primary">$1</span>'));
    ...
    const handleSearch = () => {
        ...
        if (searchQuery) {
            const searchResult: DictionaryResults[] = Object.entries(dictionary).map((dic: [string, DicType]) => {
                    ...
                    if (!Array.isArray(text[1])) {
                        ...
                        text[1].forEach((textArray) => {
                            if (textArray.includes(searchQuery)) {
                                const html = extractWords(textArray, searchQuery)
                ...
                if (foundMatch) {
                    return {section: dic[1].name, link: dic[1].link, parts, htmlParts}
                } else {
                    return null
                }
    ...
    return (
        <>
            <div className="w-full sm:w-max flex gap-0">
                <Input
                    isClearable
                    size="sm"
                    variant="bordered"
                    ...
                    onKeyDown={onKeyDown}
                    classNames={{inputWrapper: 'h-[40px] rounded-none rounded-tl-md rounded-bl-md border border-blue-200',}}
                />


                <Button onPress={handleOpenAndSearch} size="md" variant="flat" color="primary" isIconOnly
            ...
            >
                <ModalContent className='py-5 md:px-20 h-max'>
                    {(onClose) => (
                        <>
                            <ModalHeader>
                            ...
                            <ModalBody>
                                <div className='w-full flex flex-col gap-5 justify-center'>
                                    <div className='max-w-5xl'>
                                        <h1 className='text-2xl font-bold'>Search for content in the page</h1>
                                    </div>
                                    ...
                                    <div className='max-w-5xl'>
                                        <div className="w-full flex gap-0">
                                            ...
                                            />
                                            <Button onPress={handleSearch} size="md" variant="flat" color="primary"
                                                    isIconOnly
                                                    className="rounded-none rounded-tr-md rounded-br-md">
                                                <SearchIcon/>
                                            </Button>
                                        ...
                                        {foundSearch.length > 0 &&
                                            <p className='pt-2 pb-5'>{foundSearch.length} item{foundSearch.length > 1 && 's'} found
                                                for
                                                "{searchQuery}"
                                            </p>
                                        }
                                        ...
                                        {foundSearch.length === 0 && searchQuery !== "" && searchDone &&
                                            <p className='pt-5 pb-5 text-xl font-bold'>No items found
                                                for "{searchQuery}"
                                            </p>
                                        }
                                        ...
                                        {foundSearch.map((f) => {
                                            if (f) return (<div className='py-2'>
                                                <Link onClick={onClose} className='hover:text-primary text-2xl'
                                                      href={f.link}>{f.section}
                                                </Link>

                                                {f.htmlParts.map((p) => {
                                                    return (<div className='mt-b-2 mt-2'
